...
void renderSAMP ( void )
{
	...
	//	addMessageToChatWindow("%s%s %swith %s%s %s %sloaded (ƒороботка by %sMarko Friedmann%s)", szColorEnable, M0D_NAME, szColorText, szColorEnable, M0DM_NAME, M0DM_VERS, szColorText, szColorEnable, szColorText);
...
#define HUD_TEXT_TGL( x, color, text ) \
	...
	if ( isBeginRenderWIN )
	{
		...
		if (A_Set.traces && !A_Set.Tracers.empty())
		{
			...
			if (iter == A_Set.Tracers.cend())//если ебануто отображаетс€ сделать цикл for (auto& iter : A_Set.Tracers)
